...
class SHREDDataProcessor:
    ...
    def __init__(self, data, random_sensors, stationary_sensors, mobile_sensors, lags, time, compression, scaling, id):
        ...
        """
        self.n_components = compression # DAVID THIS IS FOR TESTING ONLY
        self.full_state_data = get_data(data) # full-state data where the last axis is time
    ...
    def fit_sensors(self, train_indices, method):
        ...
        Expects self.sensor_measurements to be a 2D nunpy array with time on axis 1.
        ...
        # transpose self.sensor_measurements so time on axis 0
        self.sensor_measurements = self.sensor_measurements.T
    ...
    def transform_sensor(self, method):
        ...
        if self.sensor_scaler.get(method) is not None:
            self.transformed_sensor_data[method] = self.sensor_scaler[method].transform(transformed_sensor_data)
    
    
    def fit(self, train_indices, method):
        """
        Input: train_indices and method ("random" or "sequential")
        Expects self.data to be flattened with time on axis 1.
        ...
        if self.n_components is not None:
            ...
            # un-tranpose self.full_state_data so time on axis 1
            self.full_state_data = self.full_state_data.T
            # un-tranpose full_state_data_std_scaled so time on axis 1
            full_state_data_std_scaled = full_state_data_std_scaled.T
            # rSVD
    ...
    def transform(self, method):
        """
        Expects self.full_state_data to be flattened with time on axis 1.
        ...
        if self.singular_values.get(method) is not None and self.left_singular_values.get(method) is not None and self.scaler_before_svd[method] is not None:
            transformed_data = self.scaler_before_svd[method].transform(self.full_state_data.T)
    ...
    def flatten(self, data):
        """
        Takes in a nd array where the time is along the last axis.
        Flattens the nd array into 2D array with time along axis 1.
        """
    ...
    def unflatten(self, data):
        """
        Takes in a flatten array where time is along axis 1 and the a tuple spatial shape.
