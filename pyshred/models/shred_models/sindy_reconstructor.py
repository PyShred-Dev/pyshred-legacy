...
class SINDyRECONSTRUCTOR(torch.nn.Module):
    def __init__(self, sequence, decoder, library_dim=10, poly_order=3, include_sine=False, dt=0.03, layer_norm=False):
        super().__init__()
        ...
        self.e_sindy = E_SINDy(self.num_replicates, self._sequence_model.output_size, library_dim, poly_order, include_sine).to(device)
        self.dt = dt
        self.use_layer_norm = layer_norm
        self.layer_norm_gru = torch.nn.LayerNorm(self._sequence_model.output_size)
    ...
    def forward(self, x, sindy=False):
        ...
        output = self._decoder_model(decoder_input)
...
def fit(model, train_dataset, val_dataset, batch_size=64, num_epochs=4000, lr=1e-3, sindy_regularization=1.0, verbose=True, threshold=0.5, base_threshold=0.0, patience=20, weight_decay=0.01):
    ...
    optimizer_everything = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    val_error_list = []
    patience_counter = 0
